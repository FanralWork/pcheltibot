...
async def send_joke(msg: types.Message):
    try:
        if msg.text == 'Расскажи анекдот':
            jokes = []
            with open("good_jokes.json", "r", encoding="utf-8") as read_file:
            ...
            jokes_split = []
            jokes_censured = []
            for i in jokes[random.randint(0,len(jokes))].split(' '):
            ...
            for b in range(0, len(jokes_split)):
                if jokes_split[b].translate(str.maketrans('','', string.punctuation)).casefold() in f_ck_list:
                    jokes_split[b] = f'<tg-spoiler>{jokes_split[b][0]}{(len(jokes_split[b]) - 2) * "*"}' \
                                     f'{jokes_split[b][len(jokes_split[b])-1]}</tg-spoiler>'
